...
# ==================== CONFIGURATION ====================
ALPHABET_MODEL_PATH = 'svm_alphabet.pkl'
DIGIT_MODEL_PATH = 'svm_digit.pkl'
...
# Upper inner lip: 13, Lower inner lip: 14 (vertical opening)
# Left mouth corner: 78, Right mouth corner: 308 (horizontal width)
LIP_TOP_INNER = 13
LIP_BOTTOM_INNER = 14
LIP_LEFT_CORNER = 78
LIP_RIGHT_CORNER = 308
LIP_OPEN_THRESHOLD = 0.04  # Ratio of opening/width; tune this value if needed (0.02–0.06)
...
is_detecting = False
is_lip_open = False  # Tracks current lip open/closed state for UI feedback
...
models_status = {
    "alphabet_svm": False, "digit_svm": False,
...
models_status["alphabet_svm"] = alphabet_model is not None

digit_model = load_pickle(DIGIT_MODEL_PATH)
models_status["digit_svm"] = digit_model is not None
...
def check_lip_open(landmarks, img_w, img_h):
    ...
    Calculates the vertical distance between inner upper and lower lip
    divided by horizontal mouth width. Returns True if lips are open.
    ...
    Returns:
        ...
        float: The computed mouth aspect ratio for debugging
    """
    # Get the four key points in pixel coordinates
    ...
    opening   = euclidean(top_pt, bottom_pt)   # vertical gap
    width     = euclidean(left_pt, right_pt)   # horizontal span
...
def predict_from_landmarks(landmark_sequence, mode):
    ...
    # Select models
    svm_model = alphabet_model if mode == "alphabet" else digit_model
    pca_model = pca_alphabet if mode == "alphabet" else pca_digit
    ...
    try:
        ...
        if hasattr(svm_model, 'n_features_in_'):
            expected = svm_model.n_features_in_
        ...
        # Predict
        prediction = svm_model.predict(features_for_pred)[0]
        
        # Get confidence
        if hasattr(svm_model, 'predict_proba'):
            probabilities = svm_model.predict_proba(features_for_pred)[0]
            class_idx = np.where(svm_model.classes_ == prediction)[0]
...
def generate_frames():
    ...
    global current_mode, is_detecting, current_hmm_score, is_lip_open
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            # Show lip state indicator on frame
            ...
            # Clear buffer and reset prediction when lips close
            if not lip_open:
            ...
            if len(lip_points_array) > 0:
                ...
                # Only buffer and predict when lips are open
                if lip_open:
                    # Add to buffer
...
def status():
    ...
    return jsonify({
        ...
        'alphabet_model_loaded': models_status["alphabet_svm"],
        'digit_model_loaded': models_status["digit_svm"],
        ...
        'is_lip_open': is_lip_open  # Added lip state to status
    })
...
if __name__ == '__main__':
    ...
    print("\n📋 MODEL STATUS:")
    print(f"   [{'✓' if models_status['alphabet_svm'] else '✗'}] Alphabet SVM Model")
    print(f"   [{'✓' if models_status['digit_svm'] else '✗'}] Digit SVM Model")
